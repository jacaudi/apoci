...
// TestSimpleIndexFallsBackToCachedListingOnUpstreamError exercises the
// fallback branch in handleSimplePackage (added in Task 3) that only becomes
// reachable once a package can actually be owned by upstreamOwner, which
// requires Task 4's cache-fill to have run at least once.
