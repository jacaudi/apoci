...
func TestDeletePackageVersion(t *testing.T) {
	...
	require.NoError(t, db.PutPackageTag(ctx, pkg.ID, "latest", "3.0.0", false))
...
func TestPackageTagPutGet(t *testing.T) {
	...
	require.NoError(t, db.PutPackageTag(ctx, pkg.ID, "latest", v1.Version, false))
	...
	require.NoError(t, db.PutPackageTag(ctx, pkg.ID, "latest", v2.Version, false))
	got, err = db.GetPackageTag(ctx, pkg.ID, "latest")
	require.NoError(t, err)
	require.Equal(t, v2.Version, got.Version)

	require.NoError(t, db.PutPackageTag(ctx, pkg.ID, "stable", v1.Version, false))
	tags, err := db.ListPackageTags(ctx, pkg.ID)
	require.NoError(t, err)
...
func TestPutPackageTagImmutable(t *testing.T) {
	...
	require.NoError(t, db.PutPackageTag(ctx, pkg.ID, "v1.0", v.Version, true))
	got, err := db.GetPackageTag(ctx, pkg.ID, "v1.0")
	require.NoError(t, err)
	require.True(t, got.Immutable)

	// Subsequent put on immutable tag is rejected.
	err = db.PutPackageTag(ctx, pkg.ID, "v1.0", v.Version, false)
	require.ErrorIs(t, err, ErrTagImmutable)

	// Delete on immutable is rejected.
...
func TestDeletePackageVersionImmutableBlocks(t *testing.T) {
	...
	require.NoError(t, db.PutPackageVersion(ctx, v))
	require.NoError(t, db.PutPackageTag(ctx, pkg.ID, "release", v.Version, true))
...
func TestLegacyTagImmutableTranslation(t *testing.T) {
	...
	require.NoError(t, db.PutTagWithImmutable(ctx, repo.ID, "v1.0", "sha256:zzz", true))
